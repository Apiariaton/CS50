...
// Solution of check function for this problem https://cs50.harvard.edu/x/2023/problems/5/trie/
...
bool check(char* word)
{
    ...
    for (int i = 0; i < strlen(word) /*&& check_cursor->children[check_index] != NULL*/;i++)
    {
    if (isalpha(word[i]) == 0)
    {
    return false;
    }
    check_index = tolower(word[i]) - 'a';
    if (check_cursor->children[check_index] != NULL)
    {
    check_cursor = check_cursor->children[check_index];
    }
    else
    {
    return false;
    ...
    if (check_cursor->is_word == true)
    {
    return true;
    }
    else if (check_cursor->is_word == false)
    {
    return false;
    }
    else
